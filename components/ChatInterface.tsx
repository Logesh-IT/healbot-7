...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ messages, onSend, isLowBandwidth, onGenerateReport, onClearHistory, lang }) => {
  ...
  return (
    ...
    >
      <div className={`absolute top-4 ${lang === Language.AR ? 'left-4' : 'right-4'} z-30 flex justify-between pointer-events-none`}>
        <div className="flex gap-2 pointer-events-auto">
          <button 
            onClick={() => { setShowHistory(!showHistory); setShowAnalysis(false); }}
            className={`w-10 h-10 rounded-full flex items-center justify-center transition-all shadow-md border ${showHistory ? 'bg-slate-900 text-white' : 'bg-white text-slate-500 hover:text-blue-600 border-slate-100'}`}
          ...
          <button 
            ...
            className={`w-10 h-10 rounded-full flex items-center justify-center transition-all shadow-md border ${showAnalysis ? 'bg-blue-600 text-white' : 'bg-white text-slate-500 hover:text-blue-600 border-slate-100'}`}
        ...
        <div className="pointer-events-auto">
           {messages.length > 1 && (
             <button 
              ...
              className="bg-white/80 backdrop-blur-md border border-slate-200 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-tight shadow-lg hover:bg-blue-600 hover:text-white transition-all"
             >
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-8 pt-20 space-y-10 scrollbar-hide" dir={lang === Language.AR ? 'rtl' : 'ltr'}>
        {messages.map((m) => (
          <div key={m.id} className={`flex ${m.role === 'user' ? (lang === Language.AR ? 'justify-start' : 'justify-end') : (lang === Language.AR ? 'justify-end' : 'justify-start')} animate-in fade-in slide-in-from-bottom-2 duration-500`}>
            <div className={`max-w-[85%] md:max-w-[70%] relative ${
              m.role === 'user' ? 'p-5 bg-slate-100 rounded-3xl text-slate-800' : 'p-0 text-slate-800'
            }`}>
              {m.role === 'assistant' && (
                 <div className="flex items-center gap-2 mb-4 opacity-50 text-[10px] font-black uppercase tracking-widest text-blue-600">
                    <i className="fas fa-robot"></i> HealBot AI
                 </div>
              )}
              {m.image && <img src={m.image} className="w-full max-h-[30rem] object-contain rounded-3xl mb-6 border border-slate-100 shadow-xl" />}
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <form onSubmit={handleSubmit} dir={lang === Language.AR ? 'rtl' : 'ltr'} className="flex gap-4 items-end bg-slate-100 rounded-[2rem] p-3 ring-1 ring-slate-200 focus-within:ring-2 focus-within:ring-blue-500 transition-all shadow-inner">
            ...
            <div className={`flex items-center gap-2 ${lang === Language.AR ? 'flex-row-reverse' : 'flex-row'}`}>
              ...
              <button type="submit" disabled={!input.trim() && !pendingImage} className="w-12 h-12 bg-blue-600 text-white rounded-2xl shadow-lg flex items-center justify-center hover:bg-blue-700 transition-all disabled:opacity-50">
